...
detrend = int(sys.argv[3])
agg = sys.argv[4]

root_dir = '/dartfs-hpc/rc/lab/C/CMIG'
project_dir = os.path.join(root_dir,'damages','county')
prism_dir = os.path.join(root_dir,'Data','Observations','PRISM','monthly','ppt')
imerg_dir = os.path.join(root_dir,'Data','Observations','IMERG','monthly')
cpc_dir = os.path.join(root_dir,'Data','Observations','CPC','ppt_conus')
chirps_dir = os.path.join(root_dir,'Data','Observations','CHIRPS','v3','monthly')
grace_gf_dir = os.path.join(project_dir,'data','interim','grace_gapfilled')

grace_file = os.path.join(grace_gf_dir,f'{tws_product}.nc')
if detrend==1:
    grace_file = grace_file.replace(".nc","_dt.nc")
...
for y in np.arange(2003,2025):        
    if ppt_product=='prism':
        ...
        ds = ds.resample(time='1ME').mean()
        ds.name = 'ppt'
    elif ppt_product=='era5':
        ds = xr.open_dataset(os.path.join(project_dir,'data','raw','era5_ppt_mon.nc')).sel(latitude=slice(51,23),longitude=slice(360-126,360-66)).rename({"valid_time":"time"})['tp'].sel(time=f"{y}")
        ds = 1e3*ds['time.daysinmonth']*ds
        ds = ds.resample(time='1ME').mean()
        ds.name='ppt'
    ds = fix_lat_lon(ds)
